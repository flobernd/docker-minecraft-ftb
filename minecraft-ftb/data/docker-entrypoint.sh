...
fetch_modpack_info() {
    local response
    response=$(curl --fail --connect-timeout 30 --max-time 30 --retry 3 --retry-delay 5 --no-progress-meter "https://api.feed-the-beast.com/v1/modpacks/public/modpack/$1") || exit $?
...
get_and_run_installer() {
    ...
    if [ "$(id -u)" = "0" ]; then
        find /var/lib/minecraft \( ! -user minecraft -o ! -group minecraft \) -exec chown minecraft:minecraft {} +
        chmod 0700 /var/lib/minecraft
    fi

    # Download the installer
    local content_type
    content_type=$(curl --fail --connect-timeout 30 --max-time 120 --retry 3 --retry-delay 5 --no-progress-meter -w '%{content_type}' -o "${pack_installer}" "${pack_url}") || return $?
